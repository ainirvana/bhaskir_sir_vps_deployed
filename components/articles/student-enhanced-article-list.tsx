...
interface StudentEnhancedArticleListProps {
  articles: Article[];
  viewMode?: 'grid' | 'list';
...
export function StudentEnhancedArticleList({ 
  articles, 
  viewMode = 'grid', 
  pagination, 
  onPageChange, 
  loading = false 
}: StudentEnhancedArticleListProps) {
      ...
      <CardHeader className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          <CardTitle className="text-lg font-bold line-clamp-2 flex-1">
            <Link href={`/student/articles/${article.id}`} className="hover:text-blue-600 transition-colors">
              {article.title}
            </Link>
          </CardTitle>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1 text-xs text-muted-foreground">
            ...
            {formatDate(article.published_date || article.scraped_at || new Date().toISOString())}
          </div>
        </div>
      </CardHeader>
